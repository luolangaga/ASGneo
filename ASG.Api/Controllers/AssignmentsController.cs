...
namespace ASG.Api.Controllers
{
    ...
    public class AssignmentsController : ControllerBase
    {
        ...
        public AssignmentsController(IMatchRepository matchRepo, IEventRepository eventRepo) { _matchRepo = matchRepo; _eventRepo = eventRepo; }
        ...
        public async Task<ActionResult<IEnumerable<AssignmentDto>>> My()
        {
            ...
            foreach (var ev in events)
            {
                ...
                foreach (var m in matches)
                {
                    ...
                    {
                        if (dict.TryGetValue(role, out var uidObj) && (uidObj?.ToString() ?? string.Empty) == userId)
                        {
                            var pay = dict.TryGetValue("payPerMatch", out var p) ? SafeInt(p) : 0;
                            var pos = role.Contains("commentator") ? "Commentator" : role.Contains("director") ? "Director" : "Referee";
                            list.Add(new AssignmentDto
                            {
                                MatchId = m.Id,
                                EventName = ev.Name,
            ...
            return Ok(list.OrderBy(x => x.EventName).ThenBy(x => x.MatchTitle));
        }
