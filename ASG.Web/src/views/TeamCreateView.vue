...
import { generateTeamLogo, polishText } from '../services/ai';
...
const players = ref([
  { name: '', gameId: '', gameRank: '', description: '', playerType: 2 },
]);
...
function addPlayer() {
  players.value.push({ name: '', gameId: '', gameRank: '', description: '', playerType: 2 });
}
    ...
    <v-card>
      ...
      <v-card-text>
        ...
        <v-form @submit.prevent="onSubmit">
          ...
          <v-switch v-model="hidePlayers" class="mt-1 mb-2" color="warning" hide-details :label="'隐藏队员信息（仅主办方/赛事管理员/管理员可见）'" />
          <v-alert v-if="hidePlayers" type="warning" variant="tonal" text="开启后，他人将无法查看你队员的昵称、ID、段位与简介" class="mt-2 mb-2" />
